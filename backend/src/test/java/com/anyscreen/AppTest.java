...
public class AppTest {
    ...
    @Nested
    @DisplayName("Recording Workflow Tests")
    class RecordingWorkflowTests {
        
        @Test
        @DisplayName("Should create recording with correct parameters")
        void shouldCreateRecordingWithCorrectParameters() throws Exception {
            // Given
            ScreenInfo mockScreen = TestUtils.createMockScreenInfo();
            RecordingService mockRecordingService = mock(RecordingService.class);
            
            try (MockedStatic<RecordingService> recordingServiceStatic = mockStatic(RecordingService.class)) {
                recordingServiceStatic.when(() -> RecordingService.createDefault(any(RecordingInfo.class)))
                    .thenReturn(mockRecordingService);
                
                // When
                invokeDemoRecording(mockScreen);
                
                // Then
                recordingServiceStatic.verify(() -> RecordingService.createDefault(
                    argThat(recordingInfo -> 
                        recordingInfo.getOutputFileName().equals("media/demo_recording.mp4") &&
                        recordingInfo.getFrameRate() == 30 &&
                        recordingInfo.getScreenIndex() == 0 &&
                        recordingInfo.getWidth() == 1920 &&
                        recordingInfo.getHeight() == 1080
                    )
                ));
        ...
        @Test
        @DisplayName("Should handle different screen resolutions")
        void shouldHandleDifferentScreenResolutions() throws Exception {
            // Given
            ScreenInfo ultraWideScreen = TestUtils.createMockScreenInfo();
            RecordingService mockRecordingService = mock(RecordingService.class);
            ...
            try (MockedStatic<RecordingService> recordingServiceStatic = mockStatic(RecordingService.class)) {
                ...
                // When
                invokeDemoRecording(ultraWideScreen);
                
                // Then
                recordingServiceStatic.verify(() -> RecordingService.createDefault(
                    argThat(recordingInfo -> 
                        recordingInfo.getWidth() == 3440 &&
                        recordingInfo.getHeight() == 1440 &&
                        recordingInfo.getScreenIndex() == 1
                    )
                ));
            }
        }
    ...
    private void invokeDemoRecording(ScreenInfo screenInfo) throws Exception {
        Method demoRecordingMethod = App.class.getDeclaredMethod("demoRecording", ScreenInfo.class);
        demoRecordingMethod.setAccessible(true);
        demoRecordingMethod.invoke(null, screenInfo);
    }
